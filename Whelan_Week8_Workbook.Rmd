...
NU_FPL_data<-read.csv("C:/Users/Kerri/OneDrive/Documents/CMB523/NU_FPL.csv", header = T, sep = ",")
print(NU_FPL_data)

names<-NU_FPL_data$pokemon #save pokemon names to a new variable
first_letters<-substr(names,1,1) #get first character of pokemon names
other_letters<-substr(names,2,nchar(names)) #however long it is, give 2 to however long it is (rest of the pokemon names #w/o first character)
first_letters<-toupper(first_letters) #capitalize just first letter

#paste(first_letters,other_letters,sep = "") is alternative to paste0
firsts_are_capped<-paste0(first_letters,other_letters) #concatenates first letter back to the rest
NU_FPL_data$pokemon<-firsts_are_capped #replaces original column with new data


print(NU_FPL_data[order(NU_FPL_data$pokemon),])

#NU_FPL_data$pokemon<-paste0(toupper(substr(NU_FPL_data$pokemon,1,1)),substr(NU_FPL_data$pokemon,2,nchar(NU_FPL_data$pokemon))) can be done on one line for efficiency but they both run the same

write.table(NU_FPL_data,file="NU_FPL_data.tsv",sep = "\t",quote = F,row.names = F)

NU_FPL_data<-read.csv("C:/Users/Kerri/OneDrive/Documents/CMB523/NU_FPL.csv",header = T,sep = "\t")


PU_data<-read.delim("C:/Users/Kerri/OneDrive/Documents/CMB523/PU_Ladder.tsv", header = T)
print(PU_data)

names<-PU_data$pokemon
first_letters<-substr(names,1,1)
other_letters<-substr(names,2,nchar(names))
first_letters<-toupper(first_letters)

firsts_are_capped<-paste0(first_letters,other_letters)
PU_data$pokemon<-firsts_are_capped


print(PU_data[order(PU_data$pokemon),])

#PU_data$pokemon<-paste0(toupper(substr(PU_data$pokemon,1,1)),substr(PU_data$pokemon,2,nchar(PU_data$pokemon))) can be done on one line for efficiency but they both run the same

write.table(PU_data,file="PU_data.tsv",sep = "\t",quote = F,row.names = F)
PU_data<-read.csv("C:/Users/Kerri/OneDrive/Documents/CMB523/PU_Ladder.tsv",header = T,sep = "\t")
```
...
fizzbuzz<-function(toCheck,fizzNum,buzzNum){
  result<-ifelse(
    ...
    tocheck %% fizzNum == 0 & tocheck %% buzzNum == 0,
    "FizzBuzz", #returns Fizzbuzz if divisible by fizzNum and buzzNum
    
    ifelse(
      tocheck %% fizzNum == 0,
      "Fizz", #if not divisible by both, returns Fizz if divisible only by fizzNum 
      
      ifelse(
        tocheck %% buzzNum == 0,
        "Buzz", #if not divisible by fizzNum, returns Buzz if divisible only by buzzNum
      )
    )
  )
...
#test function using example from above
toCheck<-1:266
fizzNum<-3
buzzNum<-5
print(fizzbuzz(toCheck, fizzNum, buzzNum))
