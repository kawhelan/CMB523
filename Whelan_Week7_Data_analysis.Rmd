...
sample_data_vector<-sample(data_vector, 100) #randomly selects 100 row numbers from data_vector and saves to new vector
...
rows <- nrow(trainDat) #gets the number of rows in the dataset

for(i in 1:rows) { #loops through each row in the dataset
  
  prediction <- trainDat$Prediction[i] #gets the value of in the prediction column for whatever row we're on
  
  diagnosis <- trainDat$Diagnosis[i] #ditto for diagnosis column
...
print(mismatches) #aaaaaannndd it doesn't work :(
...
ANSWER 4: Need to get that code working in order to answer
...
print(mismatches) #really can't see why this loop doesn't work
