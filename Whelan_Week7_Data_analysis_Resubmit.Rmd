...
FILENAME<-"C:/Users/Kerri/OneDrive/Documents/CMB523/breastFNA.csv" 
...
data_vector<-c(1:nrows) #vector containing all numbers between 1 and number of rows
print(data_vector)

sample_data_vector<-sample(data_vector, 100, replace = F) #randomly selects 100 row numbers from data_vector and saves to new vector
print(sample_data_vector)

testDat<-all_breast_data[sample_data_vector,] #testDat contains 100 random rows from all_breast_data
trainDat<-all_breast_data[-sample_data_vector] #trainDat<-contains all other rows
...
ANSWER 1: Specifying and object as a factor tells R to think of it as a categorical variable with different levels. 
...
ANSWER 2: The mtry value with the best accuracy was 11.
...
mismatches <- 0 #keeps track of the number of mismatches found

rows <- nrow(testDat) #gets the number of rows in the dataset

for(i in 1:rows) { #loops through each row in the dataset
  
  prediction <- testDat$Prediction[i] #gets the value of in the prediction column for whatever row we're on
  
  diagnosis <- testDat$Diagnosis[i] #ditto for diagnosis column
                                  
  if(prediction != diagnosis) { #checks if prediction does not match diagnosis
    
    mismatches <- mismatches + 1 #increases the count by a unit of 1 every time a mismatched value is found
    
  }
  
} 

print(mismatches) #now it works! replaced trainDat with testDat per feedback
...
ANSWER 4: I believe this is telling me that there were no mismatches, therefore, no bias.
...
ANSWER 4: It would be important to consider all of the implications of false positives versus false negatives in order to make a decision. 
...
ANSWER 5: We would have a 50% chance of being accurate on each guess.
...
if(!"caret"%in%installed.packages()){install.packages("caret", dependencies = TRUE)}
if(!"ada"%in%installed.packages()){install.packages("ada")}
library(caret)
library(ada)
...
#train an ada model that uses 3 boosting variants with stochastic gradient boosting, whatever that means. 
model<-train(Diagnosis~.,
            data=trainDat,
            method ='ada',
            trControl=trainControl(method = "cv",number = 5)
...
testDat$Diagnosis<-factor(testDat$Diagnosis) #convert the diagnosis into a factor in the test data

testDat$Prediction<-predict(model, newdata = testDat) #make the predictions and store them in a new prediction column 
...
print(mismatches) #now we're getting somewhere
...
ANSWER 6: This one makes you think about the pros and cons of using AI in healthcare. The risks of bias, misdiagnosis, or underdiagnosis are issues, regardless of whether the decisions are being made by machines of humans.
