...
```{r}
n= 150000000 #pick a large number for N, around one-hundred million or so
res = runif(n) + 1
```

QUESTION 1: What does the above code do?

ANSWER 1: "runif()" is a function that generates random numbers from a uniform distribution over the interval [0,1) by default. Adding 1 shifts the distribution up by 1 unit, so the interval would now be [1,2). The code assigns "res" as a vector with 150 million random numbers ranging from 1 to 2.  
...
iterations <- 3
times <- numeric(iterations) #creates vector to store time taken by each of 3 iterations

#run the segment of code 3 times using For Loop 
start_time <- Sys.time() #records start time and saves
for(j in 1:iterations){ #loop should iterate 3 times
  
  log_sum<- 0 #stores the current sum, starting at 0
 
  #add logarithms of "res" elements for to "log_sum" for each iteration
  for (i in 1:length(res)){ 
    log_sum <- log_sum + log(res[i])
  }
  end_time <- Sys.time() #records end time and assign to variable

#calculate and store time taken for this iteration
  times[j]<- end_time - start_time

print(paste(j,"result:", log_sum, "time:", times))

}

#print the average of three runs
average_time <- mean(times) #calculates average time
print(average_time)
...
ANSWER 2:
"i" is the variable that takes on the values 1 through the length of the vector "res". It represents the current value in an iteration that is updated in each step of the loop. The value on the first iteration is the first element in the vector, and the value it takes on for each subsequent iteration is the next element in the vector. 
...
#apparently this method should be faster, but it's slower for me, which means I have done something
...
ANSWER 3: "i" represents the row index of sequencing_matrix. "j" represents the column index of sequencing matrix.
the code checks if the value at each position (i,j) in sequencing_matrix is "a" and if so, replaces it with "A"
...
ANSWER 4: The "a" wasn't replaced in the second code - am I doing something wrong? I would guess we're only calling the functions once in this code, as opposed to several times in the first code
...
print(time) #why is this not working? Getting error: Error in `-.POSIXt`(end_time, start_time) : 
...
ANSWER 5: Does the loop execute for each row in sequencing_matrix? If so, 1000 times.
...
time <- end_time - start_time
#print how long it took!
##GAH!! why won't my timer work in the last couple chunks??
```
...
ANSWER 6: I can't tell yet because my timer only works sometimes, even though I used the same code for all
...
nrows <-nrow(sequencing_matrix) 
#nrows() #obtains number of rows
ncols<-ncol(sequencing_matrix) 
#ncols() obtains number of columns
for(i in nrows){ #should start loop that iterates over rows of vector
  for(j in ncols){ #should start loop that iterates over columns of vector
    if(sequencing_matrix[i,j] == "T") #should check if element in current location is "T", but now I'm stuck trying to actually count them
  }
}
