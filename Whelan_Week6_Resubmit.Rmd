...
ANSWER 6: Method 2 ran the fastest (0.0477972 sec). Method 1 was the slowest (1.167572 min.), so method 2 was about 1500 times faster than method 1. 
...
nrows <-nrow(sequencing_matrix) #nrows() #obtains number of rows
ncols<-ncol(sequencing_matrix) #ncols() obtains number of columns
tcount<-0

for(i in 1:nrows){ #should start loop that iterates over rows of vector
  for(j in 1:ncols){ #should start loop that iterates over columns of vector
    if(sequencing_matrix[i,j] == "T") #checks if element in current location is "T"
      tcount<-tcount+1 #if "T" is found, adds 1 to the value
  }
}
print(tcount)
#yay!
