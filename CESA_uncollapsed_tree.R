##CESA uncollapsed tree with bootstrap values####
...
#target <- which(as.numeric(tree$node.label) < 0) #position is list of low bootstrap
...
ggtree(tree2, aes(color=z$species)) + 
  ...
  xlim(0,3) + geom_nodelab(nudge_x = -.08, nudge_y = .5) +#define plot limits
  theme(legend.position='none') #remove the legend

ggsave("CESAtree1.png", width = 8, height = 8)